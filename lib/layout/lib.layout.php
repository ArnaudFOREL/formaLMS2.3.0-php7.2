...
class Layout
{
    ...
    {
        //if(isset($GLOBALS['page']) ) //&& $GLOBALS['page'] instanceof StdPageWriter || $GLOBALS['page'] instanceof onecolPageWriter || $GLOBALS['page'] instanceof LmsPageWriter | $GLOBALS['page'] instanceof emptyPageWriter
            return $GLOBALS['page']->getContent($zone_name);
        //else return 'Problème de chargement de contenu';
    ...
    public static function cart()
    {
        ...
        $cart = new Learning_Cart();

        $html = '<div id="cart_box" class="cart_box" style="display:' . ($cart->cartItemCount() == 0 ? 'none' : 'inline') . ';">';
        $html .= '<a id="cart_action" href="index.php?r=cart/show"><span>' . Lang::t('_SHOPPING_CART', 'catalogue')
            . ' <sub class="num_notify" id="cart_element">' . $cart->cartItemCount() . '</sub></span></a></div>';
    ...
    public static function PrepareInclude($whichLayout)
    {
        ...
        switch ($whichLayout) {
            case self::LAYOUT_HOME:
                $retArray['jsAddons'] = JQueryLib::loadJsAddons($minimized, array(), 'datepicker');
                $retArray['cssAddons'] = JQueryLib::loadCssAddons($minimized, array(), 'datepicker');
