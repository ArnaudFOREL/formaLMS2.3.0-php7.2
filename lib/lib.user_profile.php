...
class UserProfile {
	...
	function initUrlManager($std_query) {
		...
		$urlMng = new UrlManager();

		$this->_url_man = $urlMng->getInstance();
    ...
    function notifMsg($viewer = false)
	{
		$this->setViewer($viewer);
	...
	function getUserTeacherProfile($viewer = false, $link_to = false) {
		...
		$pl_man = $GLOBALS['platform_manager'] = new PlatformManager();
...
class UserProfileViewer {
    ...
    function homeUserProfile($picture = false, $viewer = false, $intest = false) { //crea la parte del profilo riguardante la foto e i certificati/messaggi
        ...
        if ($unread_num > 0 && $perm_message) {
            ...
            $html .= '<div class="col-xs-4"><a class="btn btn-default" href="index.php?r=message/show">' . Lang::t('_MESSAGES', 'standard') . '<b class="num_notify"><i style="font-size:.78em">' . $this->user_info['NOTIF'] . '</i></b></a></div>';
        }
        if ($unread_num == 0 && $perm_message) {
...
class UserProfileData {
	...
	function setCacheForUsers($arr_user) {
		...
		$mcu = new Man_CourseUser();
		$teachers = $mcu->getUserWithLevelFilter(array('4', '5', '6', '7'), $arr_user);
	...
	function isTeacher($id_user) {
		...
		$mcu = new Man_CourseUser();
		$re = $mcu->getUserWithLevelFilter(array('4', '5', '6', '7'), array($id_user));
