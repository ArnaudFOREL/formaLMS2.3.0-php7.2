...
class RequestHandler {
    ...
    public function run($ajax=false){
        ...
        if ($this->valid()){
            ...
            if (!$ajax){
                if($GLOBALS['page'] && $GLOBALS['page'] instanceof StdPageWriter || $GLOBALS['page'] instanceof onecolPageWriter || $GLOBALS['page'] instanceof emptyPageWriter) $GLOBALS['page']->add(ob_get_contents(), 'content');
            ob_clean();
            } else {
