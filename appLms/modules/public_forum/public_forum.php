    ...
    if ($can_notify) {
        ...
        $text_inner .= ' <a href="index.php?modname=public_forum&amp;op=message&amp;notify=1&amp;idThread=' . $id_thread . '&amp;ini=' . $ini_page . '" '
            . (! $is_notify ?
                ...
                . '<img src="' . getPathImage () . 'standard/notify.png" alt="' . $lang->def ('_NOTIFY') . '" /> ' . $lang->def ('_NOTIFY_ME_THREAD') . '</a> '
                :
                'title="' . $lang->def ('_UNNOTIFY_ME_THREAD_TITLE') . '">'
                . '<img src="' . getPathImage () . 'standard/unnotify.png" alt="' . $lang->def ('_UNNOTIFY') . '" /> ' . $lang->def ('_UNNOTIFY_ME_THREAD') . '</a> '
            ) . '';
    }
    if ($mod_perm) {
...
function forumDispatch ($op)
{
    ...
    $url_man = new UrlManager();
    $url_man->getInstance ('forum');
