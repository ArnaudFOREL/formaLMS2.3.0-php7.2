...
function userCourseList(&$url, $use_tab = true, $page_add = true) {
	...
	if(!empty($subscription['course'])) {
		...
		$manForum = new Man_Forum();
		
		$id_course_list = array_keys($subscription['course']);

		$orgMgt = new OrganizationManagement($id_course_list);
		...
		$advices = $manAdvice->getCountUnreaded( getLogUserId(), $id_course_list, $last_access_courses );
		
		// retrive unreaded forum messages ---------------------------------------------------------------------
		
		$forums = $manForum->getCountUnreaded( getLogUserId(), $id_course_list, $last_access_courses );
		
		// retrive new lesson ----------------------------------------------------------------------------------
		
		$org_chart = $orgMgt->getCountUnreaded( getLogUserId(), $id_course_list, $last_access_courses );
		
		if(!empty($path_courses['all_items'])) {
			$needed_info_for = array_diff($path_courses['all_items'], $id_course_list);
		}
		
		$first_is_scorm = $orgMgt->objectFilter($id_course_list, 'scormorg');
	...
	{
		$access = $manCourse->canEnterCourse($cinfo);
...
function dashmycourse(&$url, $lang, &$subscription, $cinfo, $index) {
	...
	if(!isset($subscription['edition'][$cinfo['idCourse']])) {
		...
		// not editon
		//if($cinfo['level'] >= 4) {
		switch ($cinfo['course_status']) {
			case 0:
				$cinfo['course_status'] = 'CST_PREPARATION';
				break;

			case 1:
				$cinfo['course_status'] = 'CST_AVAILABLE';
				break;

			case 2:
				$cinfo['course_status'] = 'CST_EFFECTIVE';
				break;

			case 3:
				$cinfo['course_status'] = 'CST_CONCLUDED';
				break;

			case 4:
				$cinfo['course_status'] = 'CST_CANCELED';
				break;
		}

		switch ($cinfo['user_status']) {
			case 0:
				$cinfo['user_status'] = '_CUS_RESERVED';
				break;

			case 1:
				$cinfo['user_status'] = '_CUS_WAITING_LIST';
				break;

			case 2:
				$cinfo['user_status'] = '_CUS_CONFIRMED';
				break;

			case 3:
				$cinfo['user_status'] = '_CUS_SUBSCRIBED';
				break;

			case 4:
				$cinfo['user_status'] = '_CUS_BEGIN';
				break;

			case 5:
				$cinfo['user_status'] = '_CUS_END';
				break;
		}
